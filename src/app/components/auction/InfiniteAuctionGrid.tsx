// src/app/components/auction/InfiniteAuctionGrid.tsx
'use client';
  ...
  if (items.length === 0) {
    return (
      <div className="bg-white border rounded-2xl p-12 text-center space-y-3">
        ...
        <p className="text-xs text-slate-500">Try adjusting or resetting your selected search criteria.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
          ...
          >
            <div>
              <div className="relative aspect-square w-full bg-slate-100">
                ...
                <div
                  className={`absolute top-3 left-3 text-white font-bold text-[10px] px-2.5 py-1 rounded-full backdrop-blur-sm ${
      ...
      <div ref={observerRef} className="py-6 text-center">
        {loading && <p className="text-xs font-bold text-purple-600 animate-pulse">Loading more auctions...</p>}
        {!nextCursor && items.length > 0 && (
          <p className="text-xs text-slate-400 font-semibold">You have reached the end of the marketplace.</p>
        )}
      </div>
    </div>
  );
