  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 p-3 bg-slate-100 rounded-xl">
        <img src={seller.avatar} alt={seller.name} className="w-10 h-10 rounded-full border border-slate-300" />
        <div>
          <span className="text-[10px] font-bold text-slate-400 block uppercase">Listed By Seller</span>
      ...
      {!isEnded ? (
        <div className="bg-purple-950 text-white p-4 rounded-2xl flex justify-between items-center shadow-inner">
          <div>
            <span className="text-xs font-bold text-purple-300 uppercase block">Time Remaining</span>
            <span className="text-2xl font-black font-mono tracking-wider text-emerald-400">{timeLeft}</span>
          </div>
        ...
        <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl space-y-2">
          ...
          {winner ? (
            <div className="flex items-center gap-3 pt-2 border-t border-amber-200/60">
              <img src={winner.avatar} alt={winner.name} className="w-8 h-8 rounded-full" />
